...
@interface LocationShareViewController () <RCLocationShareObserver, MKMapViewDelegate,HeadCollectionTouchDelegate>
...
- (void)viewDidLoad {
    ...
    self.headCollectionView = [[HeadCollectionView alloc] initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, 95)
                                                           participants:[self.locationShareProxy getParticipants]
                                                          touchDelegate:self];
    ...
    CLLocation *currentLocation = [self.locationShareProxy getLocation:[RCIMClient sharedRCIMClient].currentUserInfo.userId];
    if (currentLocation) {
        __weak LocationShareViewController *weakSelf = self;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.locationShareProxy addLocationShareObserver:self];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [self.locationShareProxy removeLocationShareObserver:self];
...
- (BOOL)quitButtonPressed {
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        [__weakself.locationShareProxy quitLocationShare];
    }];
    return YES;
}
...
- (void)setLocationShareProxy:(id<RCLocationShareProxy>)locationShareProxy {
    _locationShareProxy = locationShareProxy;
    [_locationShareProxy addLocationShareObserver:self];
}

#pragma mark - RCLocationShareObserver
- (void)onLocationShareStatusChange:(RCLocationShareStatus)status {
    if ([self.locationShareProxy getStatus] == RC_LOCATION_SHARE_STATUS_INCOMING ||
        [self.locationShareProxy getStatus] == RC_LOCATION_SHARE_STATUS_IDLE) {
        [self dismissViewControllerAnimated:YES completion:^{
        }];
    }
...
- (void)onReceiveLocation:(CLLocation *)location fromUserId:(NSString *)userId {
    __weak typeof(&*self) __weakself = self;
        ...
        CLLocation * cll=[self.locationShareProxy getLocation:userId];
